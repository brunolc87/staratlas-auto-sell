...
const makeV0 = (args: CompileV0Args) => {
...
describe('transaction', () => {
  ...
  it('can simulate stuff', async () => {
    const funder = keypairToAsyncSigner(Keypair.generate());
    await airdrop(connection, funder.publicKey());
    const keys = new Array(20).fill(0).map(() => Keypair.generate());
    const mint = keypairToAsyncSigner(Keypair.generate());
    const mintIx = createMint(mint, 0, funder.publicKey(), null);
    await buildSendAndCheck(mintIx, funder, connection);
    const createAtaIxs = keys.map((key) => {
    ...
    const lut = (
      await createAndExtendAddressLookupTable(
        ...
        funder,
        funder,
        [...keys.map((key) => key.publicKey), mint.publicKey()],
        undefined,
        {
          awaitNewSlot: true,
        }
    ...
    const afterIxs = createAtaIxs.slice(0, 9);
    // createAtaIxs.unshift(...COMPUTE_TEST_INSTRUCTION_RETURNS);
    const builder = new TransactionBuilder({
      connection,
      feePayer: funder,
      lookupTables: lookupTable ? [lookupTable] : [],
      afterIxs: afterIxs.map((ix) => {
        return {
          ix,
          computeEstimate: 100,
          maxTraceCount: 5,
    ...
    builder.add(createAtaIxs);

    console.log('instructions: ', builder.instructions.length);

    const txn = (await builder.buildNextOptimalTransaction())._unsafeUnwrap();
    console.log('iterations: ', builder.lastTryIterations);
    const sim = await connection.simulateTransaction(txn.transaction);
    console.log('Error: ', sim.value.err);

    const rebuilt = (await builder.rebuildLast())._unsafeUnwrap();
    console.log('rebuilt iterations: ', builder.lastTryIterations);

    const sendRes = await sendTransaction(rebuilt, connection, {
      sendOptions: { skipPreflight: true },
    });
    console.log(sendRes);
    console.log('remaining: ', builder.instructions.length);
    const next = (await builder.buildNextOptimalTransaction())._unsafeUnwrap();
    const res2 = await sendTransaction(next, connection);
    console.log(res2);
    console.log('iterations: ', builder.lastTryIterations);
  });

  it('can create lookup tables', async () => {
    const funder = keypairToAsyncSigner(Keypair.generate());
    console.log("Funder's public key: " + funder.publicKey().toBase58());
    await airdrop(connection, funder.publicKey(), LAMPORTS_PER_SOL * 10);
    const keys = new Array(50).fill(0).map(() => Keypair.generate().publicKey);
    const lut = (
      await createAndExtendAddressLookupTable(
        connection,
        funder,
    ...
    const lookupTable = (await connection.getAddressLookupTable(lut)).value;
    expect(lookupTable).not.toBeNull();
  });
  ...
  it('can use the builder', async () => {
    const funder = keypairToAsyncSigner(Keypair.generate());
    await airdrop(connection, funder.publicKey(), LAMPORTS_PER_SOL * 10);
    const from = keypairToAsyncSigner(Keypair.generate());
    await airdrop(connection, from.publicKey(), LAMPORTS_PER_SOL * 10);
    const builder = new TransactionBuilder({
      connection,
      feePayer: funder,
    });
    const transfers = new Array(100)
      .fill(0)
      .map(() => transfer(from, Keypair.generate().publicKey, 1000000));
    builder.add(transfers);
    let count = 0;
    for await (const txnRes of builder.optimalTransactions()) {
      const txn = txnRes._unsafeUnwrap();
      const result = await sendTransaction(txn, connection);
      count++;
    }
    expect(count).toBe(6);
  });
